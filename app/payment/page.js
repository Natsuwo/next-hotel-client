...
export default async function PreviewPage({ searchParams }) {
  ...
  const reservation = await getReservationByStripeSessionId(
    session.accessToken,
    session_id
  );

  if (!reservation?.id) return notFound();
  if (parseInt(reservation?.invoice?.guest_id) !== parseInt(session?.user?.id))
    return notFound();
  ...
  if (
    ...
    await updateStatusReservation(session.accessToken, session_id, "paid");
